...
% Info.T_fin(itrial).tImageOn = tImageOn - Info.StartTime;
  if strcmp(Info.T_fin(itrial).task, 'oddball')
        ...
        % iso spread = 1 -> target, iso spread = 2 -> nontarget

        % if(mod(itrial, P.BreakAfternTrials) == 1 || itrial == 1)
        %     if ~contains(Info.T_fin(itrial).category, 'target') & strcmp(Info.T_fin(itrial+1).category, 'target')
        %         Info.T_fin(itrial).iso_spread = 1;
        %         Info.file_list_iso_target{end + 1} = Info.T_fin(itrial).filename;
        %     elseif ~contains(Info.T_fin(itrial).category, 'nontarget') & strcmp(Info.T_fin(itrial+1).category, 'nontarget')
        %         Info.T_fin(itrial).iso_spread = 2;
        %         Info.file_list_iso_nontarget{end + 1} = Info.T_fin(itrial).filename;
        %     else
        %         Info.T_fin(itrial).iso_spread = 0;
        %     end
        % 
        ...
        % if(mod(itrial, P.BreakAfternTrials) == 1 || itrial == 1)
        %     if ~contains(Info.T_fin(itrial).category, 'target') & contains(Info.T_fin(itrial+1).category, 'target')
        %         Info.T_fin(itrial).iso_spread = 1;
        %         Info.file_list_iso{end + 1} = Info.T_fin(itrial).filename;
        %     else
        %         Info.T_fin(itrial).iso_spread = 0;
        %     end
        % 
        ...
        % 
        % else
        %     if ~contains(Info.T_fin(itrial-1).category, 'target') & contains(Info.T_fin(itrial+1).category, 'target')...
        %             & ~contains(Info.T_fin(itrial).category, 'target')
        %         Info.T_fin(itrial).iso_spread = 1;
        %         Info.file_list_iso{end + 1} = Info.T_fin(itrial).filename;
        %     elseif contains(Info.T_fin(itrial-1).category, 'target') & ~contains(Info.T_fin(itrial+1).category, 'target')...
        %             & ~contains(Info.T_fin(itrial).category, 'target')
        %         Info.T_fin(itrial).iso_spread = 1;
        %         Info.file_list_iso{end + 1} = Info.T_fin(itrial).filename;
        %     else
    ...
    elseif strcmp(Info.T_fin(itrial).task, 'memory')
        ...
        %     for f = 1:length(Info.file_list_iso)
        %         if strcmp(Info.T_fin(itrial).filename, Info.file_list_iso{f})
        %             Info.T_fin(itrial).iso_spread = 1;
        %             break;
        %         else
        %             Info.T_fin(itrial).iso_spread = 0;
        ...
        % if strcmp(Info.T_fin(itrial).cond, 'old')
        %     for f = 1:length(Info.file_list_iso_target)
        %         if strcmp(Info.T_fin(itrial).filename, Info.file_list_iso_target{f})
        %             Info.T_fin(itrial).iso_spread = 1;
        %             break;
        %         else
        %             Info.T_fin(itrial).iso_spread = 0;
        %         end
        %     end
        %     for f = 1:length(Info.file_list_iso_nontarget)
        %         if strcmp(Info.T_fin(itrial).filename, Info.file_list_iso_nontarget{f})
        %             Info.T_fin(itrial).iso_spread = 2;
        %             break;
        %         else
        %             Info.T_fin(itrial).iso_spread = 0;
